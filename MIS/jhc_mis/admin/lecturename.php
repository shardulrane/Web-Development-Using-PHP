...
// Check connection
	...
	$sql="SELECT * FROM lecturereport,class_reg WHERE class_reg.Faculty ='".$name."' AND class_reg.Semester='".$q."';";
...
echo "<tr bgcolor=\"$w\">";
 ...
 echo "<td>
 <a href=\"lecturemodify.php?id=$row[ID]\" onclick=\"return confirm('Are you sure want to Update $row[Faculty] with Id $row[ID]??');\">
<button class='btn btn-default'><i class='fa fa-edit'></i> Modify</button>
</a>
<a href=\"lecturedelete.php?id=$row[ID]\" onclick=\"return confirm('Are you sure want to Delete $row[Faculty] with Id $row[ID]??');\">
